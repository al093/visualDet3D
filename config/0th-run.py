...
## path
path = edict()
path.data_path = "/home/alok/dataset/KITTI-3D/training" # used in visualDet3D/data/.../dataset
path.test_path = "/home/alok/dataset/KITTI-3D/testing" # used in visualDet3D/data/.../dataset
path.visualDet3D_path = "/home/alok/repositories/visualDet3D/visualDet3D" # The path should point to the inner subfolder
path.project_path = "/home/alok/repositories/visualDet3D/project-files" # or other path for pickle files, checkpoints, tensorboard logging and output files.
...
head_loss = edict(
    ...
    regression_weight = [1, 1, 1, 1, 1,  1,  12, 1,      1,   0.5, 0.5, 0.5, 1],
                       #[x, y, w, h, cx, cy, z, sin2a, cos2a,  w,   h,   l, alpha_score]
)
head_test = edict(
    score_thr=0.75,
    cls_agnostic = False,
...
anchors = edict(
        {
            ...
            # ^ [1.0, 1.189207115002721, 1.4142135623730951, 1.681792830507429, 2.0,
            # 2.378414230005442, 2.8284271247461903, 3.363585661014858, 4.0,
            # 4.756828460010884, 5.656854249492381, 6.727171322029716, 8.0,
            # 9.513656920021768, 11.313708498984761, 13.454342644059432]
        }
    )
